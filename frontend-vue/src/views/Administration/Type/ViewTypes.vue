    ...
    <ModalComponent title="Classe" v-if="showModal" @close="showModal = false">
      <div class="flex flex-col gap-5" v-if="typeModal === 'delete_modal'">
        ...
        <div class="flex justify-center gap-4">
          <button class="px-4 py-2 border-red-500 border-2 text-red-500 rounded hover:bg-red-500 hover:text-white transition-colors duration-200 font-semibold" @click="deleteType(typeId)">Supprimer</button>
      ...
      <div class="flex flex-col gap-5 px-4" v-if="typeModal === 'update_modal'">
        <div class="flex flex-col">
          ...
          <input name="update-classe-name" type="text" class="border-2 border-gray-500 rounded px-4 py-2 focus:border-blue-600" @change="(e) => { newTypeName = e.target.value }"/>
        </div>
        <div class="flex justify-center gap-4">
      ...
      <div class="flex flex-col gap-5 px-4" v-if="typeModal === 'add_modal'">
        ...
        <div class="flex justify-center gap-4">
          <button class="px-4 py-2 border-green-500 border-2 text-green-500 rounded hover:bg-green-500 hover:text-white transition-colors duration-200 font-semibold" @click="addType(newTypeName)">Ajouter</button>
          <button class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded font-semibold" @click="showModal = false">Annuler</button>
        </div>
      </div>
    </ModalComponent>
        ...
        <table class="w-full p-4">
          ...
          <tr v-for="type in listeTypes" :key="type.id" class="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 flex space-x-2 justify-center">
              <button class="border-red-600 border-2 text-red-500 hover:text-white hover:bg-red-600 shadow hover:scale-105 duration-200 p-1 rounded-full" @click="openModal(type.id, 'delete_modal', showModal)">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                </svg>
              </button>

              <button class="border-blue-600 border-2 text-blue-500 hover:text-white hover:bg-blue-600 shadow hover:scale-105 duration-200 p-1 rounded-full" @click="openModal(type.id, 'update_modal', showModal)">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                </svg>
              </button>

              <button class="border-amber-600 border-2 text-amber-500 hover:text-white hover:bg-amber-600 shadow hover:scale-105 duration-200 p-1 rounded-full" @click="getData(type.id)">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
export default {
  ...
  methods: {
    ...
    async deleteType(type_id) {
      try{
        const response = await fetch(`http://localhost:3000/types/${type_id}`, {
          method: "DELETE",
          headers: {
    ...
    async updateType(type_id, new_type_name) {
      const data = {
        type_id, new_type_name
      };
    ...
    openModal(type_id, type_modal, showModal) {
      this.typeId = type_id;
